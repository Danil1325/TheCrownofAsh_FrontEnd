...
import { attributeLabels, classes, combineAttributes, races, type AttributeKey, type CharacterClass, type Race, wrapIndex } from './characterCreationData'
    ...
    <main className="character-creation" style={{ backgroundImage: `url("${background}")` }}>
      <section className="character-creation__main" aria-label="Character creation">
        ...
        <section className="creation-carousel" aria-label={`${step} selection`}>
          <button className="game-button carousel-arrow carousel-arrow--left" type="button" aria-label="Previous" onClick={() => move(-1)}><img src={leftArrow} alt="" /></button>
          <div className="carousel-cards">
            <CarouselCard item={cards[wrapIndex(selectedIndex - 1, cards.length)]} position="side" direction="left" />
            <CarouselCard item={cards[selectedIndex]} position="center" direction="center" selectedFrame={selectedFrame} />
            <CarouselCard item={cards[wrapIndex(selectedIndex + 1, cards.length)]} position="side" direction="right" />
          </div>
          <button className="game-button carousel-arrow carousel-arrow--right" type="button" aria-label="Next" onClick={() => move(1)}><img src={rightArrow} alt="" /></button>
        </section>
        <DisplayMenu race={selectedRace} characterClass={step === 'class' ? selectedClass : undefined} name={characterName} onNameChange={setCharacterName} />
...
function CarouselCard({ item, position, direction, selectedFrame: frame }: { item: Race | CharacterClass; position: 'side' | 'center'; direction: 'left' | 'center' | 'right'; selectedFrame?: string }) {
  const image = 'selectionImage' in item ? item.selectionImage : item.image
  return <div className={`carousel-card carousel-card--${position} carousel-card--${direction}`}><img className="carousel-card__image" src={image} alt={item.name} />{frame && <img className="carousel-card__selected-frame" src={frame} alt="" aria-hidden="true" />}</div>
}
...
function DisplayMenu({ race, characterClass, name, onNameChange }: { race: Race; characterClass?: CharacterClass; name: string; onNameChange: (name: string) => void }) {
  return <section className="display-menu" style={{ backgroundImage: `url("${displayMenu}")` }} aria-label="Character summary">
    ...
    <div className="display-menu__details">
      ...
      <div className="display-menu__resistances"><strong>RESISTANCES:</strong><span>{race.resistances.map((resistance) => `${resistance.icon} ${resistance.value}`).join('  ')}</span></div>
